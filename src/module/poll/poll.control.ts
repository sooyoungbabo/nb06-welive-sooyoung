...
async function getList(req: Request, res: Response, next: NextFunction) {
  ...
  const { polls, totalCount } = await pollService.getList(req.user.id, query);
  res.status(200).json({ polls, totalCount });
}

async function get(req: Request, res: Response, next: NextFunction) {
  const pollId = req.params.pollId as string;
  const poll = await pollService.get(req.user.id, pollId);
  res.status(200).json(poll);
}

async function patch(req: Request, res: Response, next: NextFunction) {
  const pollId = req.params.pollId as string;
  const poll = await pollService.patch(req.user.id, pollId, req.body);
  res.status(200).send({ message: '정상적으로 수정되었습니다.' });
}

async function del(req: Request, res: Response, next: NextFunction) {
  const pollId = req.params.pollId as string;
  const poll = await pollService.del(req.user.id, pollId);
